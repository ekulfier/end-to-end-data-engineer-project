...
if spark.catalog.tableExists('dev_catalog.gold.dim_model'):
    df_sink = spark.sql('''
    SELECT dim_model_key, Model_ID, model_category
    FROM dev_catalog.gold.dim_model
    ''')
...
if incremental_flag == '0':
    ...
    max_value_df = spark.sql("select max(dim_model_key) from dev_catalog.gold.dim_model")
...
# Incremenatl RUN
if spark.catalog.tableExists('dev_catalog.gold.dim_model'):
    delta_tbl = DeltaTable.forPath(spark, "abfss://dev-catalog@adbcdadventustorageaccuc.dfs.core.windows.net/gold/dim_model")
...
# Initial RUN
    ...
    df_final.write.format("delta") \
        .mode('overwrite') \
        .option("path", "abfss://dev-catalog@adbcdadventustorageaccuc.dfs.core.windows.net/gold/dim_model") \
        .saveAsTable("dev_catalog.gold.dim_model")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from dev_catalog.gold.dim_model
